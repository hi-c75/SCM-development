...
void main()
{
	...
	while(1){
		...
		while(mode==0&t==0) {
			...
			if(flag==3&k>=5){t=1;k=0;num+=1;flag=0;memcpy(InitPassword,TempPassword,sizeof(InitPassword));mode=0;WriteI2C(TempPassword,0X00,6);}			
		}
		...
		if(mode==0) {		
			...
			while(mode==0&t==1){	
				...
				if(flag==3&k>=5){
					for(i=0;i<num;i++){
						ReadI2C(CurrentPassword,0X00+6*num,6);
						for(j=0;j<6;j++){
					...
					if(flag_d==0){
						...
						while(n_alert==3){
							...
							while(flag==3&k>=5){
								if(flag==3&k>=5){
									ReadI2C(CurrentPassword,0X00,6);
									for(j=0;j<6;j++){
		...
		if(mode==2){ 
			while(mode==2){
				...
				if(flag==3){
					...
					itoa(num,string,10);
					show_string(0X0E,strcat(string,"="));
					Show_string2(start_line);
					while(flag!=3){		
						KeyDown(&t, &mode, &k, TempPassword, &flag);
						if(flag==2&k==0){flag=3;mode=2;k='a';}  // 返回
						InPut(&k, &flag, start_line, TempPassword);
						if(flag==3&k<5&k!='a'){flag=0;k=0;Show_string1(try_again);Show_string2(try_again2);delay_s(1);Show_string1(username);show_string(0X0E,strcat(string,"="));Show_string2(start_line);} //  密码位数不足
						if(flag==3&k>=5&k!='a'){
							for(i=0;i<num;i++){
								ReadI2C(CurrentPassword,0X00+6*num,6);
								for(j=0;j<6;j++){
