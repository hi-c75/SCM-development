	...
	while(1){
		...
		if(mode==0) {		
			...
			while(mode==0&t==1){	
				...
				if(flag==3&k>=5){
					for(i=0;i<num;i++){
						Read(CurrentPassword,7*i,6);delay_ms(100);
						for(j=0;j<6;j++){
					...
					if(flag_d==0){
						...
						while(n_alert==3){
							...
							while(flag==3&k>=5){
								...
								if(flag_d==0){Show_string2(Er_try);delay_s(1);Show_string2(start_line);flag=0;flag_c==0;k=0;}
								else {n_alert=0,k=0;Show_string1(name);Show_string2(start_line);}
							}
						}
					}
			...
			while(mode==1){
				...
				if(flag==3){
					...
					while(flag!=3){							
						...
						if(flag==3&k>=5&k!='a'){
							for(i=0;i<num;i++){
								Read(CurrentPassword,7*i,6);
								for(j=0;j<6;j++){
									if(CurrentPassword[j]==TempPassword[j]){flag_d=1;flag_c=7*i;}
									else{flag_d=0;break;}
								}
								if(flag_d==1)break;
							}
							if(flag_d==0){Show_string2(Er_try);delay_s(1);Show_string2(start_line);flag=0;k=0;}else {
								...
								while(flag!=3){							
									...
									if(flag==3&k>=5&k!='a'){t=1;k=0;flag=0;flag=3;mode=0;Write(TempPassword,flag_c,6);Show_string2(ResetOK);delay_s(1);}
								}
							}
						}
					}
				...
				if(flag==3){
					...
					while(flag!=3){		
						...
						if(flag==3&k>=5&k!='a'){
							...
							else {mode=0;k=0;num+=1;Write(TempPassword,7*num,6);Show_string2(right_a);delay_s(1);}
							}
						}
					flag=0;
					k=0;
