...
void main()
{
	...
	while(1){
		...
		while(mode==0&t==0) {
			...
			if(flag==3&k>=5){t=1;k=0;num+=1;flag=0;memcpy(InitPassword,TempPassword,sizeof(InitPassword));mode=0;WRNB(TempPassword,0X00,6);}			
		}
		...
		if(mode==0) {		
			...
			while(mode==0&t==1){	
				...
				if(flag==3&k>=5){
					for(i=0;i<num;i++){
						RDNB(CurrentPassword,0X00+6*(num-1),6);
						for(j=0;j<6;j++){
							if(CurrentPassword[j]==TempPassword[j]){flag_d=1;}
							else{flag_d=0;break;}
							show_number(0x49+j,TempPassword[j]);delay_s(1);
						}
						if(flag_d==1)break;
					}
					if(flag_d==0){
			...
			while(mode==1){
				...
				if(flag==3){
					...
					while(flag!=3){							
						...
						if(flag==3&k>=5&k!='a'){
							for(i=0;i<num;i++){
								RDNB(CurrentPassword,0X00+6*num,6);
								for(j=0;j<6;j++){
									if(CurrentPassword[j]==TempPassword[j]){flag_d=1;flag_c=0X00+6*num;}
									else{flag_d=0;break;}
								}
								if(flag_d==1)break;
							}
							if(flag_d==0){Show_string2(Er_try);delay_s(1);Show_string2(start_line);flag=0;k=0;}else {
								...
								while(flag!=3){							
									...
									if(flag==3&k>=5&k!='a'){t=1;k=0;flag=0;flag=0;mode=0;WRNB(TempPassword,flag_c,6);Show_string2(ResetOK);}
								}
							}
						}
					}
				...
				if(flag==3){
					...
					while(flag!=3){		
						...
						if(flag==3&k>=5&k!='a'){
							...
							else {mode=0;k=0;num+=1;WRNB(TempPassword,0X00+6*num,6);Show_string2(right_a);delay_s(1);}
							}
						}
					flag=0;
					k=0;
